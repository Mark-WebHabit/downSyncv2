...
import { schoolSupplies } from "../assets/schoolsupplies_faltfiledb_local";
import Icon from "react-native-vector-icons/FontAwesome";

import MainContainer from "../components/MainContainer";
import { Context } from "../DataContext";

const SchoolSupplies = ({ navigation }) => {
  const { speak } = useContext(Context);
  const animatedValues = schoolSupplies.map(() => new Animated.Value(1));
      ...
      >
        <View style={styles.container}>
          <View style={styles.row}>
            {schoolSupplies.slice(0, 3).map((part, index) => {
              return (
          ...
          <View style={styles.row}>
            {schoolSupplies.slice(3, 7).map((part, index) => {
          ...
          <View style={styles.row}>
            {schoolSupplies.slice(7, 10).map((part, index) => {
